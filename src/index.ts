...
import { writeGraphHtml } from "./graph-html.ts";
...
await Promise.all([
	...
	writeFile("graph.json", JSON.stringify(graphPayload, null, 2), "utf-8"),
	writeGraphHtml("graph.html", graphPayload),
]);

console.log(
	`Wrote googlesuper_tools.json (${toolsGoogle.length}), github_tools.json (${toolsGithub.length}), tools_unified.json (${unified.length}), graph.json + graph.html (${graph.edges.length} edges)`,
);
